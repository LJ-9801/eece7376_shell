...
void ExecuteCommand(struct Command *command){
    ...
    // declare file descriptors and pid
    int fd[2];
    int in = 0;
    pid_t pid;
    ...
    for (int i = 0; i < command->num_sub_commands; i++) {
        ...
        if (pid == 0) {
            ...
            if (i == 0 && command->stdin_redirect != NULL) {
                ...
                if (fd == -1) {
                    // if open returns, an error occurred, print error message and exit
                ...
                if (dup2(fd, STDIN_FILENO) == -1) {
                    // if dup2 returns, an error occurred, print error message and exit
        ...
        // if we are in the parent process
            ...
            // if the command is in the background
            ...
            // if the command is not in the background
            } else {
...
int main() {
    ...
    while (1) {
        ...
        // print out command data structure, for debugging purposes
